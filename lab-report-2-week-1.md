...
Type in your password and your terminal should look like this. (FYI Nothing will actually show up when you are typing your password).

### Step3: Trying Some Commands
Try the following commands:
- cd (won't do anything but is used to change your current directory.)
- ls (used to list all files in the currect directory)
- ls -a (used to list all files including hidden files in the current directory)
- cp /home/linux/ieng6/cs15lfa22/public/hello.txt ~/ (copies the hello.txt file to the home directory)
- cat /home/linux/ieng6/cs15lfa22/public/hello.txt (prints the contents of the text file "hello.txt")
...
To move a file from a local server to the remote server follow these steps. First exit the remote server and create a new file name test.txt.
Type a message within this text file. 

After, go back to your terminal and type the command `scp test.txt cs15lfa22zz@ieng6.ucsd.edu:~/` where the zz is once again replaced with your specific login. This sends the text file to the remote server. 
![image](https://user-images.githubusercontent.com/114527221/195942033-d58daaf4-e5e7-43bf-9855-e1c41ec7fa8b.png)

And if you login to the remote server and type `ls -a` you will see that the file has been added there.
...
From here you must copy the public key to the .ssh directory of your server. Follow the following steps:
- (on client) ssh cs15lfa22zz@ieng6.ucsd.edu
- (on server) mkdir .ssh (and then logout)
- (on client) scp /Users/joe/.ssh/id_rsa.pub cs15lfa22zz@ieng6.edu:~/.ssh/authorized_keys (You use your username and the path you saw in this command line)
This should allow you to login to the remote server without a password.
...
Other shortcuts include `ssh cs15lfa22@ieng6.ucsd.edu "ls"` or `cp WhereAmI.java OtherMain.java; javac OtherMain.java; java WhereAmI` (to run a program with such as "WhereAmI.java")
